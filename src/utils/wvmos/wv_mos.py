# the following code is adapted from https://github.com/AndreevP/wvmos
...
class Wav2Vec2MOS(nn.Module):
    def __init__(self, path, freeze=True, cuda=True):
        ...
        self.load_state_dict(
            extract_prefix("model.", torch.load(path, weights_only=False)["state_dict"])
        )
        self.eval()
        self.cuda_flag = cuda
        if cuda:
    ...
    def get_mos_score(self, signal: torch.Tensor) -> float:
        """
        Calculate mos score by fine-tuned wav2vec2.0 model from https://huggingface.co/facebook/wav2vec2-base

        Args:
            signal (Tensor): 16Khz audio for calculating the mos score.
        Returns:
            output (float): calculated mos score.
        """
        signal = torchaudio.functional.resample(signal, MelSpectrogramConfig.sr, 16000)
        x = self.processor(
            signal, return_tensors="pt", padding=True, sampling_rate=16000
        ).input_values
        with torch.no_grad():
