        ...
        **kwargs,
        ...
        if data is None:
            ...
            for transcription_path in tqdm(
                list((ts_path / "transcriptions").iterdir()),
                desc="Creating custom dataset",
            ):
                data.append({"transcription": transcription_path.read_text()})
        else:
            assert (
                "audio_path" in data[0].keys()
            ), "Invalid data passed to CustomDirDataset"
            for i in tqdm(range(len(data)), desc="Creating test dataset"):
                data[i]["transcription"] = ts_path / (
